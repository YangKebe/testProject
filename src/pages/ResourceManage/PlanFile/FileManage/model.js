import { queryRecords, deleteRecord, submitNew, modifyRecord, queryModalData } from './service';
...
export default {
  ...
  reducers: {
    ...
    'setDetail'(state, { payload }) {
      state.curDetail = payload.detailData.data.records;
      return JSON.parse(JSON.stringify(state));
    },
  ...
  effects: {
    ...
    *submitNew({ payload }, { call, put }) {
      ...
      if (data.retCode.status === '0000') {
        message.success("添加成功", 1);
      } else if (data.retCode.status === '9990') {
        message.error('添加失败，主键重复');
      } else {
        message.error("添加失败", 1);
      }
      yield put(routerRedux.push({
        pathname: '/BasicLayout/ResourceManage/PlanFile/FileManage',
        state: {}
      }));
      yield put({ type: 'nextPage', payload: { data: {}, whereStr: null } })
    },
