...
import { exportTable, indexClassConvertCh, indexClassConvertNum } from '/utils/utils';
...
class DataTable extends Component {
    ...
    onDelete = () => {
        ...
        selectedRows.map(item => {
            regPlanId += item.regplanId + ',';
            indexClass += indexClassConvertNum(item.indexClass) + ',';
        })
        Modal.confirm({
            title: '删除',
            content: '确认删除？',
    ...
    onRow = record => {
        return {
            onDoubleClick: () => {
                this.dispatchDetail(record);
            }
        }
    };

    dispatchDetail = record => {
        record.indexClass = indexClassConvertNum(record.indexClass);
        this.props.dispatch({
            type: `${pageTag}/goToDetail`,
            payload: {
                pathname: `${this.props.match}/detail`,
    ...
    render() {
        ...
        data.map(item => { item.indexClass = indexClassConvertCh(item.indexClass) })
            ...
            <div>
                <div style={{ marginTop: '15px', marginBottom: '5px' }}>
                    ...
                    <Button  {...btnStyle}
                        ...
                        onClick={() => {
                            this.dispatchDetail(this.state.selectedRows[0]);
                        }}
                    >修改</Button>
